...
def main(p1, p2):
    ...
    ################GRIND SIZE##########################
    # first splitting 60% out for training then 40% for validation & testing
    x_train, x_test, y_train, y_test = train_test_split(data[['roast_type','bean_type','rating','strength']].values,
    ...
    if data['rating'].values == 10.0: #retains values if ratings are 10
        ...
        saturation_prediction = knn_sat.predict(data[['roast_type','bean_type','rating','strength']].values)
        temp_prediction = knn_temp.predict(data[['roast_type','bean_type','rating','strength']].values)
        grind_prediction = knn_grind.predict(data[['roast_type','bean_type','rating','strength']].values)
